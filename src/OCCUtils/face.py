...
class DiffGeomSurface(object):
  ...
  def curvature(self, u, v):
    ...
    if not self._curvature_initiated:
      self._curvature = GeomLProp_SLProps(self.instance.surface_handle, u, v, 2,
                                          1e-7)
    ...
    if u in _domain or v in _domain:
      ...
      div = 1000
      delta_u, delta_v = (_domain[0] - _domain[1]) / div, (_domain[2] -
                                                           _domain[3]) / div
...
#===========================================================================
  ...
  def project_curve(self, other):
    ...
    if (isinstance(other, TopoDS_Edge) or isinstance(other, Geom_Curve) or
        issubclass(other, Geom_Curve)):
      # convert edge to curve
      first, last = topexp.FirstVertex(other), topexp.LastVertex(other)
      lbound, ubound = BRep_Tool().Parameter(first,
                                             other), BRep_Tool().Parameter(
                                                 last, other)
  ...
  def edges(self):
    return [
        Edge(i) for i in WireExplorer(next(self.topo.wires())).ordered_edges()
    ]
